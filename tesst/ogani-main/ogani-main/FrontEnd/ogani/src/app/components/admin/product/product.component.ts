...
export class ProductComponent implements OnInit {
  ...
  constructor(private http: HttpClient, private messageService: MessageService,private productService: ProductService,private imageService: ImageService,private categoryService:CategoryService){
  ...
  createProduct(){
    ...
    this.productService.createProduct(name,description,price,quantity,categoryId,imageIds).subscribe({
      next: res =>{
        ...
        this.showError("Vui lòng nhập đầy đủ các trường !");
      }
    })
  }
  ...
  chooseImage() {
    this.listImageChoosen = [...this.listImageChoosen, ...this.selectedImages];
    console.log(this.listImageChoosen);
    this.showImage = false;
  }
  selectImage(event: any, res: any) {
    const index = this.selectedImages.indexOf(res);
    if (index > -1) {
    ...
    this.imageChoosen = this.selectedImages.length > 0 ? this.selectedImages[0] : null;
