...
export class TagComponent implements OnInit {
  ...
  createTag(){
    ...
    this.tagService.createTag(name).subscribe({
      next: res =>{
        this.getList();
        this.showSuccess("Tạo thẻ tag thành công!");
  ...
  updateTag(){
    ...
    this.tagService.updateTag(id,name).subscribe({
      next: res =>{
        this.getList();
        this.showSuccess("Cập nhật thẻ tag thành công!");
        this.displayForm = false;
      },error: err =>{
  ...
  deleteTag(){
    ...
    this.tagService.deleteTag(id).subscribe({
      next: res =>{
        this.getList();
        this.showWarn("Xóa tag thành công!!");
        this.deleteForm = false;
      },error: err=>{
        this.showError("Không thể xóa khi đã gắn ở blog !");
        this.deleteForm = false;
      }
    })
  }
